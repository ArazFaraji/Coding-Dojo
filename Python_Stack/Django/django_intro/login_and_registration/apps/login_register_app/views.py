...
def register(request):
    ...
    if len(validate_error) > 0:
        ...
        loggedInUser = User.objects.create(first_name = request.POST['firstName'], last_name = request.POST['lastName'], email = request.POST['email'], birthday = request.POST['birthday'], password = pw_hash.decode())
